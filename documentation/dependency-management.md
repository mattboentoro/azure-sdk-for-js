...
Let's assume that a vulnerability has been found in [@typespec/ts-http-runtime] version 0.3.3 and that you use `@azure/keyvault-keys` directly. Your security scan identified this vulnerability and asks to upgrade to 0.3.4 or higher.
...
First, let's see how `@typespec/ts-http-runtime` is pulled into our dependency tree.

```
> npm ls @typespec/ts-http-runtime
...
> If you are using Yarn you can use `yarn why @typespec/ts-http-runtime` with similar results.

It looks like `@typespec/ts-http-runtime`, a dependency of `@azure/core-rest-pipeline@1.19.0` is being pulled in _transitively_ via `@azure/keyvault-keys@4.10.0`.
...
keyvault main % npm ls @typespec/ts-http-runtime
keyvault@1.0.0 /home/user/my-app
└─┬ @azure/keyvault-keys@4.10.0
  └─┬ @azure/core-rest-pipeline@1.19.0
...
As you can see `@typespec/ts-http-runtime` has been updated to 0.3.4, without having to wait for a new version of `@azure/core-rest-pipeline`.
...
Not all updates are due to security vulnerabilities. Sometimes you just want to update a transitive dependency without deleting your lockfile. In that case, `npm update @typespec/ts-http-runtime` can help. Let's see what that looks like:

```
> npm update @typespec/ts-http-runtime
...
> If you are using Yarn you can use `yarn up @typespec/ts-http-runtime` with similar results.
...
[@typespec/ts-http-runtime]: https://www.npmjs.com/package/@typespec/ts-http-runtime
